...
class ScraperService {
  ...
  private async initBrowser(service: string): Promise<Browser> {
    try {
      ...
      logger.error(`Failed to initialize browser for ${service}:`, {
        error,
      });
      throw new AppError(
  ...
  public async cgcScraper(certNumber: string): Promise<ScrapingResult> {
    ...
    try {
      ...
      await page.waitForSelector(
        'fieldset.certlookup-search__inputs input[name="certNumber"]'
      );
      await page.type(
        'fieldset.certlookup-search__inputs input[name="certNumber"]',
        certNumber
      ...
      const [response] = await Promise.all([
        page.waitForResponse(
          (response) =>
            response.url().includes("cert-lookup") && response.status() === 200
        ),
        page
          .waitForSelector(".certlookup-result", {
            timeout: config.services.cgc.timeout,
          })
          .catch(() => null),
      ]);
      ...
      const gameInfo = await page.evaluate(() => {
        const result: GameInfo = {
          ...
          platform:
            document.querySelector(".platform")?.textContent?.trim() ?? null,
          certificationDate:
        ...
        if (popReport) {
          ...
          result.populationReport.data = {
            totalGraded:
              document.querySelector(".total-graded")?.textContent?.trim() ??
              null,
            higherGrades:
